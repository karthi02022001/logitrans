...
<div class="container-fluid">
    <div class="row mb-3">
        <div class="col-md-12">
            <h2>{{ __('Edit Shipment') }}: {{ $shipment->shipment_number }}</h2>
        </div>
    </div>
            ...
            <form action="{{ route('admin.shipments.update', $shipment) }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="shipment_number" id="shipment_number" class="form-control @error('shipment_number') is-invalid @enderror" value="{{ old('shipment_number', $shipment->shipment_number) }}" placeholder="SH-2025-0001" required>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="vehicle_type_id" id="vehicle_type_id" class="form-select @error('vehicle_type_id') is-invalid @enderror" required>
                            ...
                            @foreach($vehicleTypes as $type)
                                <option value="{{ $type->id }}" {{ old('vehicle_type_id', $shipment->vehicle_type_id) == $type->id ? 'selected' : '' }}>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" step="0.01" min="0" name="cargo_weight" id="cargo_weight" class="form-control @error('cargo_weight') is-invalid @enderror" value="{{ old('cargo_weight', $shipment->cargo_weight) }}" placeholder="0.00">
                        @error('cargo_weight')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="priority" id="priority" class="form-select @error('priority') is-invalid @enderror" required>
                            <option value="normal" {{ old('priority', $shipment->priority) == 'normal' ? 'selected' : '' }}>{{ __('Normal') }}</option>
                            <option value="high" {{ old('priority', $shipment->priority) == 'high' ? 'selected' : '' }}>{{ __('High') }}</option>
                            <option value="urgent" {{ old('priority', $shipment->priority) == 'urgent' ? 'selected' : '' }}>{{ __('Urgent') }}</option>
                        </select>
                        @error('priority')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="row">
                    <div class="col-md-12">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-save"></i> {{ __('Update Shipment') }}
                        </button>
